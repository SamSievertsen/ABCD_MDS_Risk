...
kproto_results <- readRDS(file = "../../results/main_analysis/kproto_clustering_results.rds")

# Read in individual silhouette scores
silhouette_data <- read.csv("../../results/main_analysis/kproto_individual_silhouette_scores.csv")

# Read in k grouped silhouette means
silhouette_scores <- read.csv("../../results/main_analysis/kproto_silhouette_means.csv")

# Read in elbow plot data
elbow_plot_data <- read.csv("../../results/main_analysis/kproto_elbow_plot_data.csv")
...
Results from the `risk_group_clustering` script suggest it is likely that optimal *k* = 5. Re-iterating the most important information here, this determination was made because the elbow plot suggests a natural elbow wherein reduction in total within sum of squares values levels off after *k* = 5 (*note*: a similar tapering is observed after *k* = 6, but the distinction between the two *k* values will be made in the discussion of the silhouette index results): 
...
In addition to the elbow plot, wherein *k = 5* is observed to exhibit the qualities of an elbow, the *k* grouped mean silhouette scores indicate that *k = 5* is the third most optimal clustering solution (μ = 0.182; following *k = 6* closely, wherein μ = 0.187): 
...
At this point, despite the slightly greater reduction in total within sum of squares values, it is not evident whether *k = 5* is more optimal than *k = 6*. This becomes more clear, though, when we examine the density plot of silhouette scores at the individual subject within-cluster level: 
...
At *k = 5*, the tight grouping of clusters about silhouette = 0.20 for clusters 1, 2, 3, and 5 and relatively good (albeit lower and more spread) performance of cluster 4 (about silhouette = 0.05) is vastly superior to *k = 6*. 

At *k = 6*, clusters 1, 2, 4, and 5 perform well (i.e., are grouped about silhouette = 0.25), but clusters 3 and 6 perform worse than cluster 4 in the *k = 5* solution, with cluster 3 actually mean centered below 0 (indicating that most subjects in that cluster would have been more accurately grouped into another cluster). 

As such, when considering the results of both the elbow plot and silhouette index, it is likely that optimal *k* = 5. 
...
write.csv(clustered_risk_variable_data, "../../data/data_processed/clustered_risk_variable_data.csv", row.names = FALSE)
