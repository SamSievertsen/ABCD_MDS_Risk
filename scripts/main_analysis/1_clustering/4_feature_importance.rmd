...
params:
  ...
  shadow_B: 1000             # reps for shadow-null baseline (shadow-max +, optionally p-values)
...
shadow_baseline <- function(kp, data_dt, base_assign,
  ...
  mode <- match.arg(mode)
  if (!is.null(seed)) set.seed(seed + 101L)
...
run_seed <- function(seed_idx) {
    ...
    mcr_df$p_emp <- vapply(seq_len(nrow(mcr_df)), function(i) {
      vals <- sb_feat_list[[i]]$vals  # these are shadow(f) draws with pre-permute
      obs <- mcr_df$mcr_mean[i]      # observed = mean MCR after permuting real f
...
if (params$mode %in% c("auto", "aggregate")) {
  ...
  #7.1.3 Aggregate across seeds: mean MCR, SD, win-rate (feature-specific stability)
  stab <- all_df |>
    group_by(feature) |>
    summarise(
      mcr_mean = mean(mcr_mean, na.rm = TRUE),
      mcr_sd = sd(mcr_mean,  na.rm = TRUE),
      win_rate = mean(pass_shadow_feat, na.rm = TRUE),   # <-- feature-specific
      .groups  = "drop") |>
    arrange(desc(mcr_mean)) |>
