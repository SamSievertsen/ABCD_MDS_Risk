...
if (params$mode %in% c("auto", "aggregate")) {
  ...
  for (i in seq_len(n_folds)) {
    ...
    bi <- tryCatch(bst_cv$best_iteration, error = function(...) NULL)
    if (is.null(bi) || length(bi) == 0L || is.na(bi) || bi < 1L) bi <- params$xgb_nrounds
  ...
  xgboost::xgb.save(bst, file.path(paths$shap_dir, "xgb_surrogate.ubj"))

  #8.8 SHAP global importances (mean |SHAP|) for descriptive triangulation
  sv <- shapviz::shapviz(bst, X_pred = X_mat, X = X_df)
  
  #8.8.0 Save exact SHAP artifacts for downstream manuscript figures
  saveRDS(sv, file.path(paths$shap_dir, "xgb_surrogate_shapviz.rds"))
  saveRDS(list(X_df = X_df, X_mat = X_mat, y = y, features_used = features_used), file.path(paths$shap_dir, "xgb_surrogate_inputs.rds"))
  
  #8.8.0.1 Save surrogate model in modern portable format
  xgboost::xgb.save(bst, file.path(paths$shap_dir, "xgb_surrogate.ubj"))
  ...
  #8.8.4 Save SHAP outputs
  saveRDS(list( X_df = X_df, X_mat = X_mat, feature_names = colnames(X_mat)), file.path(paths$shap_dir, "xgb_surrogate_shap_inputs.rds"))
