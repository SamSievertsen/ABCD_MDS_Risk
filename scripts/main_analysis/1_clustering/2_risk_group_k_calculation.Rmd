...
#2.5.1 Compute total sum of squares (TSS) once
overall_mean <- colMeans(scaled_data)
tss <- sum(rowSums((scaled_data - overall_mean)^2))
...
cluster_metrics <- purrr::map_dfr(
  ...
  function(k) {
    ...
    #2.5.2.1 Pull cluster assignments for this k
    cl <- kp_list[[paste0("k", k)]]$cluster

    #2.5.2.2 Within‐cluster SSD
    wcss_per_cluster <- tapply(
      seq_along(cl), cl,
      function(idxs) {
        sum(rowSums((scaled_data[idxs, ] - overall_mean)^2))
      }
    )
    wcss_total <- sum(wcss_per_cluster)

    #2.5.2.3 Get RMSSD
    rmssd <- sqrt(mean(wcss_per_cluster / table(cl)))

    #2.5.2.4 Get R squared
    r2_val <- (tss - wcss_total) / tss

    #2.5.2.5 log to detailed log
    cat(
      sprintf("%s|METRICS|%s|k=%d|RMSSD=%.4f|R2=%.4f\n", format(Sys.time(), "%Y-%m-%d %H:%M:%OS3"),
        scaling_method, k, rmssd, r2_val),
      file = log_file, append = TRUE
    )

    #2.5.2.6 Return a one‐row tibble with results
    tibble(
      k = k,
      RMSSD = rmssd,
      R2 = r2_val)
...
#2.6.1 Function to one‐hot encode categorical variables + pipe in scaled numerics
...
#2.6.3 Loop over each selected k and create UMAP plots + diagnostic plots
for (k in selected_k) {

  #2.6.3.1 Extract data & cluster labels
  kp_obj <- kp_list[[paste0("k", k)]]
  data_k <- kp_obj$data
  cluster_labels <- factor(kp_obj$cluster)

  #2.6.3.2 Prepare config for UMAP (slight deviation from default to see density in blob better)
  df_umap <- prepare_mixed_data(data_k)
  cfg <- umap.defaults
  cfg$n_neighbors <- 15
  cfg$min_dist <- 0.1
  cfg$spread <- 1.5
  ...
  #2.6.3.3 Generate the k specific UMAP results
  umap_res <- umap(df_umap, config = cfg)
  
  #2.6.3.4 Store the UMAP results in a dataframe
  umap_df <- data.frame(
  ...
  #2.6.3.5 Plot UMAP scatter
  p_umap <- ggplot(umap_df, aes(x = UMAP1, y = UMAP2, color = Cluster)) +
    geom_point(size = 1.5, alpha = 0.6) +
    labs(
      title = glue::glue("UMAP scatter (k = {k})"),
      subtitle = glue::glue("n_neighbors = {cfg$n_neighbors}, min_dist = {cfg$min_dist}"),
      x = "UMAP 1",
      y = "UMAP 2") +
    theme_minimal(base_size = 14) +
    theme(legend.title = element_text(size = 10))
  
  #2.6.3.6 Print the k-specific UMAP plot
  print(p_umap)

  #2.6.3.7 Plot blob density + overlay
  p_blob <- ggplot(umap_df, aes(x = UMAP1, y = UMAP2)) +
    geom_density_2d(alpha = 0.3, color = "gray40") +
    geom_point(aes(color = Cluster), size = 1.2, alpha = 0.7) +
    labs(
      title = glue::glue("UMAP blob (k = {k})"),
      subtitle = "Density contours + cluster overlay") +
    theme_minimal(base_size = 14) +
    theme(legend.title = element_text(size = 10))
  
  #2.6.3.8 Print the blob density plot
  print(p_blob)

  #2.6.3.9 Highlight each cluster with a unique scatter plot fixed over the UMAP results
  clus_ids <- sort(unique(cluster_labels))
  cluster_plots <- lapply(clus_ids, function(cl) {
    df2 <- umap_df
    df2$highlight <- ifelse(df2$Cluster == cl, paste("Cluster", cl), "Other")
    ggplot(df2, aes(x = UMAP1, y = UMAP2)) +
      geom_point(aes(color = highlight, alpha = highlight), size = 1) +
      scale_color_manual(values = c("Other" = "gray80", paste("Cluster", cl) = "red")) +
      scale_alpha_manual(values = c("Other" = 0.3, paste("Cluster", cl) = 0.8)) +
      labs(title = glue::glue("k = {k} → Highlight: Cluster {cl}")) +
  ...
  #2.6.3.10 Arrange and print up to 6 per plots row (dependent upon k)
  if (length(cluster_plots) <= 6) {
    do.call(gridExtra::grid.arrange, c(cluster_plots, ncol = 2))
  } else {
    print(cluster_plots)
  }
}
...
#3.2.1 Filter dir_tbl to only the indices we have
dir_tbl <- dir_tbl %>% filter(index %in% indices)

#3.2.2 Join the cached index results
...
- Among the `r nrow(summary_tbl)` validation indices we were able to run, `r sum(summary_tbl$k_opt == consensus_k)` (`r paste(summary_tbl$label[summary_tbl$k_opt == consensus_k], collapse = ", ")`) also vote for *k* = `r consensus_k`, while the others (`r paste0(summary_tbl$label[summary_tbl$k_opt != consensus_k], " (k=", summary_tbl$k_opt[summary_tbl$k_opt != consensus_k], ")", collapse = ", ")`) peak at neighboring values 
