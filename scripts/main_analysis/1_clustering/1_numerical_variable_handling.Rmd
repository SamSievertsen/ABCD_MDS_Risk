...
options(future.globals.maxSize = 1e9)  # allow large objects in futures
...
# Fallback SLURM job name when running locally
SLURM_JOB_NAME <- Sys.getenv("SLURM_JOB_NAME", "local_numeric_scaling")

# Determine where to write the detailed log (from the SLURM wrapper or default)
log_file <- Sys.getenv(
  "DETAILED_LOG",
...
kproto_dir <- file.path(REPO, "data_processed", "kproto_results")
# Where to stash the full validation_kproto results
validation_dir <- file.path(REPO, "data_processed", "validation_results")
...
all_results <- future_map_dfr(
  ...
  function(m) {
    ...
    if (file.exists(proto_chk) && all(file.exists(val_chks))) {
      ...
      cat(sprintf(
        "%s|PROTO_SKIP|%s|loaded from checkpoint\n",
