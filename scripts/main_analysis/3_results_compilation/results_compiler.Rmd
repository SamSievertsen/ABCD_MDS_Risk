...
emp_cum_by_wave <- bd_pp_model %>%
  ...
  #F5.4.1.2 Roll up to cumulative incidence: 1 - Π(1 - h_j)
...
#S3.1.7 Right‑censor the nested suicidality person‑period data at each participant’s first onset per outcome so Table S3 counts are computed on a proper first‑onset risk set
su_pp_model <- su_pp_model %>%
  arrange(
...
if (!exists("su_pp_model")) {
  ...
  su_pp_model <- nested_pp %>%
    ...
    #S3.2.1 Filter data to only contain model‑ready rows
...
#S3.3 Helper functions for display options
#S3.3.1 Make sure cluster values are listed as desired
...
#S3.3.2 Create vectors for the preferred assessment timepoint values and their temporal ordering
time_map <- c("ses-02A"="2y","ses-04A"="4y","ses-06A"="6y")
time_levels <- c("2y","4y","6y")

#S3.3.3 Format how descriptive statistics should be displayed
#S3.3.3.1 Mean +- SD
fmt_mean_sd <- function(x) {
  x <- x[is.finite(x)]
  if (!length(x)) return("\u2014")
...
#S3.4 Ensure the working person-period dataframe exists and is structured as desired
...
#S3.5 Clean up demographics data in ever-BD set by cluster
#S3.5.1 Collapse data to one row per person per wave to avoid duplicating across outcomes
demo_base <- pp %>%
  dplyr::distinct(participant_id, time, cluster_disp, sex, site_factor, age_mid)

#S3.5.2 Compile base demographic descriptive stats for the ever-BD person-period set by cluster
demo_counts <- demo_base %>%
  dplyr::group_by(time, cluster_disp) %>%
  dplyr::summarise(
    n_pp = dplyr::n(),
...
#S3.5.3 Create a formatted age in years descriptive stat row by cluster
age_row <- demo_counts %>%
  dplyr::transmute(time, cluster_disp, Row = "Age in years, Mean (SD)", val = age_mean_sd)

#S3.5.4 List the N participants at each timepoint interval by cluster
n_rows <- demo_counts %>%
  tidyr::pivot_longer(c(n_pp, n_part), names_to = "what", values_to = "val_raw") %>%
  dplyr::mutate(
    Row = dplyr::case_when(
...
#S3.5.5 List the sex distribution by cluster
sex_tab <- demo_base %>%
  dplyr::mutate(sex2 = dplyr::case_when(
...
#S3.5.6 Bind all of the cluster grouped N, age, and sex descriptive stats into a single dataframe
demo_rows_cs <- dplyr::bind_rows(n_rows, age_row, sex_tab) %>%
  dplyr::mutate(Section = "Demographics")

#S3.6 Create total descriptive stats across whole sample (both clusters) otherwise as above
#S3.6.1 Collapse data to one row per person per wave to avoid duplicating across outcomes
demo_counts_tot <- demo_base %>%
  dplyr::group_by(time) %>%
  dplyr::summarise(
    n_pp = dplyr::n(),
...
#S3.6.2 Compile base demographic descriptive stats for the ever-BD person-period set
n_rows_total <- demo_counts_tot %>%
  tidyr::pivot_longer(c(n_pp, n_part), names_to = "what", values_to = "val_raw") %>%
  dplyr::mutate(
    Row = dplyr::case_when(
...
#S3.6.3 Create a formatted age in years descriptive stat row
age_row_total <- demo_counts_tot %>%
  dplyr::transmute(time, cluster_disp = "Total", Row = "Age in years, Mean (SD)", val = age_mean_sd)

#S3.6.4 List the sex distribution by cluster
sex_tab_total <- demo_base %>%
  dplyr::mutate(sex2 = dplyr::case_when(
    !is.na(sex) & sex %in% c("Male","Female") ~ as.character(sex),
    TRUE ~ "Other/Unknown")) %>%
...
#S3.6.5 Bind all of the N, age, and sex descriptive stats into a single dataframe
demo_rows_total <- dplyr::bind_rows(n_rows_total, age_row_total, sex_tab_total) %>%
  dplyr::mutate(Section = "Demographics")
...
# 2) DEMOGRAPHICS (participant x wave; collapse across outcomes)
demo_base <- panel %>%
...
# Inputs:
#   - Saved GEE models with wave x cluster interaction:
...
OUT_LABS <- c(si_active="Active ideation", si_passive="Passive ideation", sa="Suicide attempt", nssi="NSSI")
...
em_or_by_wave <- function(fit) {
  ...
  w <- setNames(numeric(length(lev)), lev); w["C2"] <- 1; w["C1"] <- -1
  cmp <- emmeans::contrast(emm, method = list("C2 - C1" = w))   # one contrast per wave
