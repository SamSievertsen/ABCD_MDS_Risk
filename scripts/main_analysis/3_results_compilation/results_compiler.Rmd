...
#1.6.15 Helper to read val_robust_silhouette_k*.rds into a tidy DF if possible
...
cluster_labels <- read_or_tk(
  first_existing(
    p_analysis("cluster_labels_k2_robust.csv"),
    p_analysis("cluster_labels_k2_robust.rds")),
  tk_cols = c("participant_id","cluster")) %>% 
  ensure_participant_id()

#2.1.4 Read in BD panel data
bd_panel <- read_or_tk(first_existing(
  p_analysis("bd_panel_k2_robust.csv"),
  p_analysis("bd_panel_k2_robust.rds")), 
...
bd_pp <- read_or_tk(first_existing(
  p_analysis("bd_person_period_k2_robust.csv"),
  p_analysis("bd_person_period_k2_robust.rds")), 
...
nested_panel <- read_or_tk(first_existing(
  p_analysis("nested_suic_panel_k2_robust.csv"),
  p_analysis("nested_suic_panel_k2_robust.rds")), 
...
nested_pp <- read_or_tk(first_existing(
  p_analysis("nested_suic_person_period_k2_robust.csv"),
  p_analysis("nested_suic_person_period_k2_robust.rds")), 
  tk_cols = c("participant_id","wave","age","interval_id","cluster","ever_bd","event_si_passive","event_si_active","event_sa","event_nssi")) %>%
  ensure_participant_id()

#2.2 Map C1/C2 cluster labels to manuscript labels (C1 = Higher; C2 = Lower)
...
- Inputs: `val_robust_silhouette*` RDS/CSV (or per-k partials), UMAP embeddings (`umap_embedding_k{2,3,4}.{csv,rds}`), and optional precomputed `k_wss`.
...
em_or_by_wave <- function(fit) {
  ...
  #F6.2.2 Force contrast to be C1 - C2 (so OR = odds_C1 / odds_C2) regardless of reference
