...
#8.0 Create a helper function to get a safe count of unique participants per dataset
...
#8.1.5 Quick sizes (rows and unique IDs) for each dataset pre-filter
...
#8.1.5.1 Create a kable outlining the sizes of each dataset pre completeness filter
knitr::kable(sizes_tbl_pre, caption = "8.1A Dataset sizes (pre-filter)")

#8.2 Person-period invariants (pre-filter)
...
#8.2.2 Assess the presence of any person-period data invariants in the BD and suicidality data
...
#8.2.3 No baseline-at-risk rows for baseline-positive cases (pre-filter)
pp_baseline_viol <- function(pp, baseline_wave) {
  if (!nrow(pp)) return(0L)
...
knitr::kable(
  tibble::tibble(Dataset = c("BD person-period (pre-filter)","Suicidality person-period (pre-filter)"),
...
#8.2.4 Event<->status parity (pre-filter)
#     Define onset in panels as: status==1 AND (prior status != 1), where prior for the first Y2 wave uses baseline status
build_panel_onsets <- function(panel_df, baseline_col = NULL) {
  if (!nrow(panel_df)) return(panel_df[0, c("outcome","wave","onsets_panel")])
  panel_df %>%
    ...
    dplyr::mutate(
      ...
      onset = dplyr::case_when(
        is.na(prev_status) & !is.null(baseline_col) ~ (dplyr::coalesce(.data[[baseline_col]], 0L) == 0L & status == 1L),
        is.na(prev_status) & is.null(baseline_col) ~ (status == 1L),
...
bd_onset_panel <- build_panel_onsets(bd_panel, baseline_col = "baseline_status_bd")
su_onset_panel <- su_panel %>%
  dplyr::filter(as.character(wave) <= params$max_suic_end_wave) %>%
  build_panel_onsets(baseline_col = "baseline_status_su")
...
knitr::kable(bd_onset_compare %>% dplyr::arrange(outcome, wave),
             caption = "8.2.4 BD (pre-filter): panel-detected onsets vs person-period events (parity)")
knitr::kable(su_onset_compare %>% dplyr::arrange(outcome, wave),
             caption = "8.2.5 Suicidality (pre-filter): panel-detected onsets vs person-period events (parity)")

#8.3 Covariate coverage & centering sanity checks pre completeness filter
pp_missing <- function(df, label) {
  vars <- intersect(c("age_start","age_end","age_mid","age_mid_gmc","age_mid_cwc", "sex","site_factor","family_id","race_ethnicity"),
            names(df))
...
#8.3.3 Age monotonicity check (panel-based, pre)
...
#8.4 Cluster availability after merges pre completeness filter
cluster_cov_chk <- tibble::tibble(
  Dataset = c("BD person-period (pre-filter)","Suicidality person-period (pre-filter)","BD panel (pre-filter)","Suicidality panel (pre-filter)"),
  N_rows  = c(nrow(bd_pp), nrow(su_pp), nrow(bd_panel), nrow(su_panel)),
  Missing_cluster = c(sum(is.na(bd_pp$cluster)), sum(is.na(su_pp$cluster)), sum(is.na(bd_panel$cluster)), sum(is.na(su_panel$cluster))))
knitr::kable(cluster_cov_chk, caption = "8.4 Cluster label presence (pre-filter)")

#8.5 QC plots pre completeness filter
bd_events_by_wave <- bd_pp %>%
  dplyr::group_by(outcome, end_wave) %>%
  dplyr::summarise(events = sum(event == 1L, na.rm = TRUE), .groups = "drop")
if (nrow(bd_events_by_wave)) {
  p_bd_events <- ggplot(bd_events_by_wave, aes(x = end_wave, y = events)) +
    geom_col() +
    facet_wrap(~ outcome, scales = "free_y") +
    labs(title = "First-onset events by wave (BD, pre)",
...
su_events_by_wave <- su_pp %>%
  dplyr::group_by(outcome, end_wave) %>%
  dplyr::summarise(events = sum(event == 1L, na.rm = TRUE), .groups = "drop")
if (nrow(su_events_by_wave)) {
  p_su_events <- ggplot(su_events_by_wave, aes(x = end_wave, y = events)) +
    geom_col() +
    facet_wrap(~ outcome, scales = "free_y") +
    labs(title = "First-onset events by wave (Suicidality, pre)",
         x = "Wave (interval end)", y = "Event count") +
    theme_minimal(base_size = 13)
  print(p_su_events)
}
...
#8.6 Define model-ready filters (keep rows with observed outcome + required covariates + cluster)
bd_pp_model <- bd_pp %>%
...
restrict_panel_to_pp <- function(panel_df, pp_df) {
  if (!nrow(panel_df) || !nrow(pp_df)) return(panel_df[0, c("participant_id","outcome","wave","status")])
  keep_pairs <- pp_df %>%
...
bd_onset_panel_post <- build_panel_onsets(bd_panel_post, baseline_col = "baseline_status_bd")
su_onset_panel_post <- su_panel_post %>%
  dplyr::filter(as.character(wave) <= params$max_suic_end_wave) %>%
  build_panel_onsets(baseline_col = "baseline_status_su")

bd_onset_pp_post <- bd_pp_model %>%
  dplyr::group_by(outcome, end_wave) %>%
  dplyr::summarise(onsets_pp = sum(event == 1L, na.rm = TRUE), .groups = "drop") %>%
  dplyr::rename(wave = end_wave)

su_onset_pp_post <- su_pp_model %>%
  dplyr::group_by(outcome, end_wave) %>%
  dplyr::summarise(onsets_pp = sum(event == 1L, na.rm = TRUE), .groups = "drop") %>%
  dplyr::rename(wave = end_wave)

bd_onset_compare_post <- dplyr::full_join(bd_onset_panel_post, bd_onset_pp_post, by = c("outcome","wave")) %>%
  dplyr::mutate(diff = onsets_panel - onsets_pp)
su_onset_compare_post <- dplyr::full_join(su_onset_panel_post, su_onset_pp_post, by = c("outcome","wave")) %>%
  dplyr::mutate(diff = onsets_panel - onsets_pp)

knitr::kable(bd_onset_compare_post %>% dplyr::arrange(outcome, wave),
             caption = "8.6.5 BD (post): panel-detected onsets vs person-period events (parity)")
knitr::kable(su_onset_compare_post %>% dplyr::arrange(outcome, wave),
             caption = "8.6.6 Suicidality (post): panel-detected onsets vs person-period events (parity)")
...
#8.6.8 Compact roll-up summary (pre vs post)
rollup_tbl <- tibble::tibble(
  ...
  Value  = c(pp_bd_chk$n_multi_onsets, pp_su_chk$n_multi_onsets,
             ...
             sum(bd_onset_compare_post$diff != 0, na.rm = TRUE),
             sum(su_onset_compare_post$diff != 0, na.rm = TRUE)))
