...
#1.6.4 Create a column simplifying the timepoint/wave indicator in case of co-linearity
...
pred_by_age_by_bd <- function(fit, dat, bd_var = "bd_any_start", ages = ages_pred_vec, wave_ref = params$wave_ref) {
  ...
  age_between_vals <- as.numeric(ages_safe - grand_mean)  # raw "between" units
  scales <- get_age_scales(dat)
  
  # map raw -> model scale if needed
  ...
  n_clu <- length(lvl(dat$cluster))
  age_years_rep <- rep(round(as.numeric(ages_safe), 2), each = n_clu)
  ...
  emmeans_try <- function(bd_state) {
    ...
    at_list <- list(
      ...
      end_wave         = mean(dat$end_wave, na.rm = TRUE),
      sex              = lvl(dat$sex)[1],
  ...
  fallback_try <- function(bd_state) {
    ...
    pr <- suppressWarnings(try(stats::predict(fit, newdata = grid, type = "response", re.form = NA), silent = TRUE))
  ...
  res %>%
    dplyr::mutate(cluster  = fct_like(cluster, dat$cluster),
                  bd_state = as.integer(bd_state)) %>%
...
if (nrow(haz_df)) {
  g_haz <- ggplot(haz_df,
                  aes(x = factor(end_wave, levels = c(0,1), labels = c("Baseline","Post-baseline")),
