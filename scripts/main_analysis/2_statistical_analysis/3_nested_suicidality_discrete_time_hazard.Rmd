...
params:
  ...
  suic_pp_rds: "nested_suic_person_period_k2_z_score.rds"
  suic_pp_csv: "nested_suic_person_period_k2_z_score.csv"
  link_primary: "logit"
  wave_ref: "ses-02A"
  ages_pred: !r c()
  seed: 123
...
#1.2 Set seed and other numerical options
...
#1.4 Set relevant paths for I/O
REPO <- params$repo
DATA_DIR <- file.path(REPO, params$data_dir)
OUT_DIR <- file.path(REPO, params$out_dir)
dir.create(OUT_DIR, recursive = TRUE, showWarnings = FALSE)

#1.5 Read in relevant data
#1.5.1 Create a function to safely read in person-period data
...
#1.5.2 Read in person-period data
...
#1.6 Clean person-period data for modeling
#1.6.1 Create a vector for relevant outcomes
SUIC_OUTCOMES <- c("si_passive","si_active","sa","nssi")

#1.6.2 Filter nested person-period data to only contain outcomes of interest
su_pp <- su_pp %>% filter(outcome %in% SUIC_OUTCOMES)

#1.6.3 Ensure all data for modeling is present/complete
su_pp_model <- su_pp %>%
...
#1.6.4 Create a column simplifying the timepoint/wave indicator in case of co-linearity -- not currently used in modeling but retaining for transparency/potential future relevance
su_pp_model <- su_pp_model %>%
  mutate(post_baseline = as.integer(end_wave != "ses-02A"))

#1.6.5 Create a column for DTH interval length from ages
su_pp_model <- su_pp_model %>%
  mutate(dt_years = pmax(age_end - age_start, 1/12),  # guard: >= 1 month
         log_dt = log(dt_years))

#1.6.6 Flag and drop any rows with non-sensical interval lengths (check output)
...
#1.6.7 Set factor column reference levels
su_pp_model <- su_pp_model %>%
  mutate(
    cluster = factor(cluster, levels = c("C1","C2")), # C1 = low-risk (reference), C2 = high-risk
...
#1.6.8 Perform a dataset size sanity check
#1.6.8.1 List size of data
sizes_tbl <- tibble(
...
#1.6.8.2 Print dataset size sanity check results
knitr::kable(sizes_tbl, caption = "Dataset size check") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped","hover","condensed","responsive"))

#1.6.9 First-onset risk set per suicidality outcome: exclude baseline-positive for that outcome
#1.6.9.1 Store IDs with no baseline-positive onset for each outcome
...
#1.6.9.2 Ensure only baseline-negative for each outcome in analysis data
su_pp_model <- su_pp_model %>%
  left_join(su_baseline_ids, by = c("participant_id","outcome"))

#1.6.9.3 List n before drop 
n_before <- nrow(su_pp_model); ids_before <- n_distinct(su_pp_model$participant_id)

#1.6.9.4 Only keep baseline negative subjects in data for modeling
su_pp_model <- su_pp_model %>% filter(baseline_outcome == 0L)

#1.6.9.5 List n subjects dropped due to requirements for baseline-negative participants
message(
...
#1.6.10 Censor at the first onset (per outcome x participant) ; keeps all rows up to and including first event==1L; drops rows strictly after it. 
su_pp_model <- su_pp_model %>%
  arrange(
...
#1.6.10.1 Audit to ensure censoring was done correctly
aud <- su_pp_model %>%
...
aud %>% summarise(any_post = any(post_onset_rows > 0),
  n_ids_with_post = sum(post_onset_rows > 0))

#1.7 Empirical wave-median ages (for plotting/prediction)
...
#1.8 Ages to use for plotting/pred vector
...
#1.9 Descriptives: events by outcome x wave x cluster
...
#1.10 Post-filter missingness of model covariates
#1.10.1 List covariates of interest
covars <- c("cluster","age_mid_between","age_mid_cwc","sex","site_factor","end_wave","participant_id","family_id","bd_any_start","bd_any_end")

#1.10.2 Create a table outlining missingness for each variable if interest
...
#1.10.3 Print variable missingness table
...
* Covariates mirror BD DTH: `cluster + age_mid_between + sex + end_wave`
* As in the BD DTH models, within-person centered age (age_mid_cwc) was omitted from the primary specification due to high collinearity with wave indicators (VIF >= 50) given sparse intervals
* Random effects: Primary model is GLM; inference uses CR2 robust SEs clustered by participant. A secondary QC model attempts GLMM (glmer) with (1|participant_id) + (1|family_id) + (1|site_factor) and automatic simplification if singular/non-convergent
* Primary link: logit; sensitivity: cloglog + offset(log_dt)
...
build_formula <- function(bd_var = "bd_any_start", include_re = FALSE) {
  rhs_fixed <- paste(
    "cluster",
    "age_mid_between",
    #"age_mid_cwc",
  ...
  if (!isTRUE(include_re)) {
    return(as.formula(paste("event ~", rhs_fixed)))
  }

  rhs_re <- paste(
    rhs_fixed,
    "(1 | participant_id) + (1 | family_id) + (1 | site_factor)",
    sep = " + ")
  as.formula(paste("event ~", rhs_re))
}

fit_glm_primary <- function(formula, data, link = c("logit","cloglog")) {
  link <- match.arg(link)
  fam  <- if (link == "logit") binomial("logit") else binomial("cloglog")

  # Strip random effects if present
...
fit_glmer_secondary <- function(formula, data, link = c("logit","cloglog")) {
  link <- match.arg(link)
  fam  <- if (link == "logit") binomial("logit") else binomial("cloglog")

  # If no (1|grp) terms remain, fall back to glm
  if (length(lme4::findbars(formula)) == 0L) {
...
refit_if_boundary <- function(fit, form, data, link) {
  ...
  fit_glmer_secondary(f2, data, link = link)
}
simplify_random_effects <- function(fit, form, data, link) {
  ...
  for (grp in cand) {
    if (has_re_term(best_form, grp)) {
      ...
      m_try <- try(fit_glmer_secondary(f_try, data, link = link), silent = TRUE)
...
perf_metrics <- function(fit, data) {
  ...
  if (inherits(fit, "merMod")) {
    ...
    # McFadden pseudo-R2 as a light QC metric for GLM
    r2 <- try(performance::r2_mcfadden(fit), silent = TRUE)
    if (!inherits(r2, "try-error")) R2m <- as.numeric(r2$R2)
  }
...
pred_by_age_by_bd <- function(fit, dat, bd_var = "bd_any_start", ages = ages_pred_vec, wave_ref = params$wave_ref) {
  ...
  # Clamp requested ages to observed range (same intent as your current code)
  ...
  scales <- get_age_scales(dat)
  age_between_for_model <- if (!is.null(scales$between)) apply_age_scales(age_between_vals, scales$between) else age_between_vals
  age_cwc_for_model <- if (!is.null(scales$cwc)) apply_age_scales(0, scales$cwc) else 0

  # Factor utilities
  lvl <- function(x) levels(droplevels(x))

  # Build an at list like BD DTH does
  base_at <- list(
    cluster = lvl(dat$cluster),
    age_mid_between = age_between_for_model,
    age_mid_cwc = age_cwc_for_model,
    end_wave = factor(wave_ref, levels = lvl(dat$end_wave)),
    sex = lvl(dat$sex)[1],
    site_factor = lvl(dat$site_factor)[1])

  # Keep only variables actually used by the fit (BD DTH guard)
  mf_names <- try(names(model.frame(fit)), silent = TRUE)
  if (!inherits(mf_names, "try-error")) {
    base_at <- base_at[names(base_at) %in% mf_names]
  }

  emmeans_try <- function(bd_state) {
    at_list <- base_at
    at_list[[bd_var]] <- as.integer(bd_state)

    vc <- robust_vcov_for(fit, dat) # may be NULL
    em <- emmeans::emmeans(fit, ~ cluster | age_mid_between, at = at_list, vcov. = vc, type = "response")
    sm <- tibble::as_tibble(summary(em, infer = c(TRUE, TRUE)))
    resp <- emm_resp_col(sm); ci <- emm_ci_cols(sm)

    tibble::tibble(
      cluster = sm$cluster,
      age_years = rep(round(as.numeric(ages_safe), 2), each = length(lvl(dat$cluster))),
  ...
  fallback_try <- function(bd_state) {
    ...
    nd <- tidyr::expand_grid(
      ...
      dplyr::mutate(
        ...
        sex = lvl(dat$sex)[1],
        site_factor = lvl(dat$site_factor)[1])
    nd[[bd_var]] <- as.integer(bd_state)

    pr <- suppressWarnings(try(stats::predict(fit, newdata = nd, type = "response", re.form = NA), silent = TRUE))
  ...
  need_fallback <- inherits(out0, "try-error") || !any(is.finite(out0$pred)) ||
                   inherits(out1, "try-error") || !any(is.finite(out1$pred))

  res <- if (need_fallback) dplyr::bind_rows(fallback_try(0L), fallback_try(1L))
         else dplyr::bind_rows(out0, out1)

  res %>% dplyr::filter(is.finite(pred))
}
...
bd_contrasts_by_age <- function(fit, dat, bd_var = "bd_any_start", ages = ages_pred_vec, wave_ref = params$wave_ref) {
  ...
  at_list <- list(
    age_mid_between = age_between_for_model,
    age_mid_cwc = age_cwc_for_model,
    end_wave = factor(wave_ref, levels = levels(dat$end_wave)),
  ...
  ct <- contrast(em, list("1 vs 0" = c(-1, 1)))
  sm <- as.data.frame(summary(ct, infer = TRUE))

  tibble::tibble(
    age_years = round(as.numeric(ages), 2),
    contrast = "1 vs 0",
    OR = exp(sm$estimate),
...
hazard_by_period_by_bd <- function(fit, dat, bd_var = "bd_any_start") {
  ...
  vc <- robust_vcov_for(fit, dat)
  scales <- get_age_scales(dat)
  age_between0 <- if (!is.null(scales$between)) apply_age_scales(0, scales$between) else 0
  age_cwc0 <- if (!is.null(scales$cwc)) apply_age_scales(0, scales$cwc) else 0

  # Use the waves actually present in this subset (typically 2 here)
  w_obs <- levels(droplevels(dat$end_wave))
  if (length(w_obs) == 0) return(tibble::tibble())
  if (length(w_obs) > 2) w_obs <- w_obs[c(1, length(w_obs))]
  ...
  for (bd_state in c(0L, 1L)) {
    ...
    at_list <- list(
      cluster = levels(dat$cluster)[1],
      age_mid_between = age_between0,
      age_mid_cwc = age_cwc0,
      end_wave = factor(w_obs, levels = levels(dat$end_wave)),
      sex = levels(dat$sex)[1],
      site_factor = levels(dat$site_factor)[1])
    at_list[[bd_var]] <- bd_state

    em <- emmeans::emmeans(fit, ~ end_wave, at = at_list, vcov. = vc, type = "response")
    sm <- as.data.frame(summary(em, infer = TRUE))
    resp <- emm_resp_col(sm); ci <- emm_ci_cols(sm)

    outs[[as.character(bd_state)]] <- tibble::tibble(
      end_wave = sm$end_wave,
...
# Scaling helpers for age vars (used when we z-score age for SA only)
get_age_scales <- function(dat) {
  list(
    between = attr(dat, "age_between_scale"),
    cwc     = attr(dat, "age_cwc_scale")
  )
}
apply_age_scales <- function(x, sc) {
  if (is.null(sc) || any(!is.finite(sc))) return(x)
  (x - sc[1]) / sc[2]
}
...
for (out in OUTCOMES) {
  ...
  # scale ages for SA only (z-score), and stash scales for prediction
  if (identical(out, "sa")) {
    sb <- c(mean(dat_o$age_mid_between, na.rm = TRUE),
            stats::sd(dat_o$age_mid_between, na.rm = TRUE))
    sc <- c(mean(dat_o$age_mid_cwc,     na.rm = TRUE),
            stats::sd(dat_o$age_mid_cwc,     na.rm = TRUE))
    dat_o <- dat_o %>%
      mutate(
        age_mid_between = (age_mid_between - sb[1]) / sb[2],
        age_mid_cwc     = (age_mid_cwc     - sc[1]) / sc[2])

    # store scales so the prediction helpers can invert back to raw ages
    attr(dat_o, "age_between_scale") <- sb
    attr(dat_o, "age_cwc_scale")     <- sc
  }
  ...
  form_o <- build_formula(bd_var = "bd_any_start")
  form_o <- prune_formula_for_data(form_o, dat_o)

  # SA: avoid collinearity with post_baseline (two-wave design)
  if (identical(out, "sa")) {
    form_o <- update.formula(form_o, . ~ . - age_mid_cwc)
  }
  ...
  # ----- PRIMARY: GLM (fixed effects) -----
  fit_o <- fit_glm_primary(form_o, dat_o, link = params$link_primary)
  fits_primary[[out]] <- fit_o
  msg_model_health(fit_o, glue("{out} | primary GLM {params$link_primary}"))
  
  # ----- OPTIONAL QC: GLMM (secondary; may fail / simplify) -----
  form_re <- build_formula(bd_var = "bd_any_start", include_re = TRUE)
  form_re <- prune_formula_for_data(form_re, dat_o)
  
  # keep SA collinearity guard consistent
  if (identical(out, "sa")) {
    form_re <- update.formula(form_re, . ~ . - age_mid_cwc)
  }
  
  fit_re <- safe_or_null(fit_glmer_secondary(form_re, dat_o, link = params$link_primary))
  if (!is.null(fit_re)) {
    fit_re <- refit_if_boundary(fit_re, form_re, dat_o, link = params$link_primary)
    fit_re <- simplify_random_effects(fit_re, form_re, dat_o, link = params$link_primary)
    msg_model_health(fit_re, glue("{out} | QC GLMM {params$link_primary}"))
  }
  
  fits_qc_glmm[[out]] <- fit_re
  diag_qc_glmm[[out]] <- if (!is.null(fit_re)) safe_or_null(diag_report(fit_re) %>% mutate(outcome = out, model = "QC_GLMM", .before = 1)) else NULL
  ...
  cr2_primary[[out]] <- safe_or_null({
    cl <- if ("participant_id" %in% names(dat_o) && nlevels(droplevels(dat_o$participant_id)) >= 2) {
      dat_o$participant_id
    } else if ("family_id" %in% names(dat_o) && nlevels(droplevels(dat_o$family_id)) >= 2) {
      dat_o$family_id
    } else {
      NULL
    }
    tidy_cr2(fit_o, cluster_var = cl) %>%
  ...
  nonevents = sum(event == 0L, na.rm = TRUE))
  message(glue::glue("  Subset stats -> n={dbg$n}, ids={dbg$n_ids}, fam={dbg$n_fam}, site={dbg$n_site}, events={dbg$events}, nonevents={dbg$nonevents}"))
  fit_s1 <- fit_glm_primary(form_s1, dat_o, link = "cloglog")
  ...
  if (all(!is.na(dat_o$bd_any_end))) {
    form_s2 <- build_formula(bd_var = "bd_any_end")
    form_s2 <- prune_formula_for_data(form_s2, dat_o)
    if (identical(out, "sa")) {
     form_s2 <- update.formula(form_s2, . ~ . - age_mid_cwc)
    }
    dbg <- dat_o %>% summarise(
    n = dplyr::n(),
    n_ids = dplyr::n_distinct(participant_id),
    ...
    nonevents = sum(event == 0L, na.rm = TRUE))
    message(glue::glue("  Subset stats -> n={dbg$n}, ids={dbg$n_ids}, fam={dbg$n_fam}, site={dbg$n_site}, events={dbg$events}, nonevents={dbg$nonevents}"))
    fit_s2 <- fit_glm_primary(form_s2, dat_o, link = params$link_primary)
...
if (any(!is.na(dat_o$bd_any_start_lag1))) {
  ...
  if (can_fit_binomial(dat_lag) && has_two_levels(dat_lag$bd_any_start_lag1)) {
    form_s3 <- build_formula(bd_var = "bd_any_start_lag1")
    form_s3 <- prune_formula_for_data(form_s3, dat_lag)
    if (identical(out, "sa")) {
      form_s3 <- update.formula(form_s3, . ~ . - age_mid_cwc)
    }
    fit_s3 <- fit_glm_primary(form_s3, dat_lag, link = params$link_primary)
...
# Summarize models kept and participant SD
re_summary <- purrr::imap_dfr(fits_qc_glmm, function(m, out) {
...
knitr::kable(re_summary, digits = 6, caption = "QC GLMM fits: (1|participant_id) presence & SD")
...
if (nrow(haz_df)) {
  g_haz <- ggplot(haz_df,
              aes(x = ifelse(as.character(end_wave) == params$wave_ref, "Baseline", "Post-baseline"),
    y = hazard, fill = factor(bd_state))) +
    scale_x_discrete(limits = c("Baseline","Post-baseline")) +
...
# BD contrasts table
bdc <- if (exists("bd_contr_all") &&
           is.data.frame(bd_contr_all) && nrow(bd_contr_all) > 0) {
  bd_contr_all
} else {
  tibble::tibble()
}
if (nrow(bdc)) {
  knitr::kable(
    bdc %>% dplyr::arrange(outcome, age_years),
    digits = 3,
...
if (nrow(tbl_wald) > 0) {
  knitr::kable(tbl_wald %>% arrange(outcome, model), digits = 3,
    caption = "Fixed effects (naive Wald SEs) Odds ratios with 95% CI (QC only)") %>%
...
if (nrow(tbl_cr2) > 0) {
  knitr::kable(tbl_cr2 %>% arrange(outcome, model), digits = 3,
    caption = "Primary inference: GLM with CR2 robust SEs (clustered by participant) - Odds ratios with 95% CI") %>%
    kableExtra::kable_styling(full_width = FALSE, bootstrap_options = c("striped","hover","condensed","responsive"))
}
...
* GLM is primary (as in BD DTH) due to sparse events / 2-wave design; GLMM is attempted only as QC and often unstable (singular/convergence)
