...
# Add a post-baseline indicator to preserve the effect of the KSADS switch without a 3-4 level wave variable
bd_panel_model <- bd_panel_model %>%
  mutate(
...
#1.3 Build formula (additive; post baseline FE for wave effect of KSADS; within/between age decomposition)
#1.3.1 Choose time terms safely
rhs_time <- function(time_spec = params$time_spec, include_baseline = params$include_baseline, include_lag = params$include_lag_status) {
  stopifnot(time_spec %in% c("age","wave"))
  base <- c("cluster", "age_wave_between", "sex", "post_baseline")
  if (isTRUE(include_baseline)) base <- c(base, "baseline_status")
  if (time_spec == "age") {

    #1.3.1.1 PRIMARY: use within-person AGE; DROP wave FE
    c(base, "age_wave_cwc", "cluster:age_wave_between", if (include_lag) "lag_status")
  } else {

    #1.3.1.2 SENSITIVITY: use WAVE FE; DROP within-person age
    c(base, "wave", if (include_lag) "lag_status")
  }
}

#1.3.2 New formula builder (keeps your RE structure)
build_formula <- function(include_lag = FALSE, link = c("logit","cloglog")) {
  link <- match.arg(link)
  rhs <- c(rhs_time(params$time_spec, params$include_baseline, include_lag),
           "(1 | participant_id)", "(1 | family_id)", "(1 | site_factor)")
  as.formula(paste(params$response_var, "~", paste(rhs, collapse = " + ")))
}
...
tidy_cr2 <- function(fit, cluster_var = NULL) {
  is_glmm <- inherits(fit, "glmerMod") || grepl("binomial", family(fit)$family, ignore.case = TRUE)
  test_method <- if (is_glmm) "z" else "Satterthwaite"
  ...
  df <- as_tibble(ct); if (!"df" %in% names(df)) df$df <- NA_real_
  p_col <- intersect(c("p_Satt","p_z","p"), names(df))[1]
  df %>% mutate(term = as.character(term), OR = exp(beta), CI_lo = exp(CI_L), CI_hi = exp(CI_U)) %>%
    transmute(term, estimate = beta, SE, df, OR, CI_lo, CI_hi, p.value = .data[[p_col]])
}

#1.6 emmeans helpers
#1.6.1 Predictions across ages at a fixed wave (within-age = 0 for interpretability)
pred_grid_by_age <- function(fit, data, ages = ages_pred_vec) {
  ...
  nd <- expand.grid(
    ...
    post_baseline = stats::median(data$post_baseline, na.rm = TRUE))
  nd$age_years <- rep(ages, each = length(levels(data$cluster)))
  nd$pred <- predict(fit, newdata = nd, type = "response", re.form = NA)
  nd
}

#1.6.2 Model matrix builder that matches a fitted lme4 model (fixed effects only)
mm_for <- function(fit, newdata) {

  #1.6.2.1 Build a terms object for FIXED effects and drop the response
  tt <- stats::delete.response(terms(lme4::nobars(formula(fit))))
  
  #1.6.2.2 Use the fitted model frame to recover factor levels & contrasts
  mf_fit <- model.frame(fit)
  xlev <- lapply(mf_fit[sapply(mf_fit, is.factor)], levels)
  
  #1.6.2.3 Build a compatible newdata model frame
  mf_new <- model.frame(tt, newdata, xlev = xlev, na.action = na.pass)
  
  #1.6.2.4 Reuse contrasts from the fitted design (fallback to derive if absent)
  X_fit <- lme4::getME(fit, "X")
  contr <- attr(X_fit, "contrasts")
  if (is.null(contr)) contr <- attr(model.matrix(tt, mf_fit), "contrasts")
  model.matrix(tt, mf_new, contrasts.arg = contr)
}

#1.6.3 Wald CI on probability scale with fixed effects only (re.form = NA)
pred_with_ci <- function(fit, newdata, level = 0.95) {
  eta <- as.numeric(predict(fit, newdata = newdata, type = "link", re.form = NA))
  X <- mm_for(fit, newdata)
  V <- as.matrix(vcov(fit))
  se <- sqrt(pmax(1e-12, rowSums((X %*% V) * X)))
  z <- qnorm(1 - (1 - level)/2)
  pred <- plogis(eta)
  lo <- plogis(eta - z * se)
  hi <- plogis(eta + z * se)
  tibble(pred = pred, lo = lo, hi = hi)
}

#1.6.4 Predictions across ages at a fixed wave (within-age = 0)
pred_by_age_wald <- function(fit, dat, ages = ages_pred_vec, wave_ref = params$wave_ref) {
  ...
  nd <- expand.grid(
    ...
    post_baseline = as.integer(wave_ref != "ses-02A"))
  nd$age_years <- rep(ages, each = length(levels(dat$cluster)))
  bind_cols(nd, pred_with_ci(fit, nd)) %>%
    dplyr::select(cluster, age_years, pred, lo, hi)
}

#1.6.5 Predictions by wave at reference covariates
pred_by_wave_wald <- function(fit, dat) {
  ...
  nd <- tibble::tibble(
    ...
    age_wave_cwc = 0,
    sex = levels(dat$sex)[1])
  if (isTRUE(params$include_baseline)) nd$baseline_status <- 0
  nd <- nd %>% dplyr::mutate(post_baseline = as.integer(wave != "ses-02A"))
  dplyr::bind_cols(nd, pred_with_ci(fit, nd)) %>%
    dplyr::transmute(wave, pred, lo, hi)
}
...
#1.15 Stepwise RE simplification (keep participant; drop family THEN site if singular)
simplify_random_effects_me <- function(fit, formula, data, link = "logit") {
  di <- diag_report(fit)
  if (!isTRUE(di$is_singular) && isTRUE(di$converged)) return(fit)

  #1.15.1 Always keep participant RE
  f0 <- formula

  #1.15.2 Drop family first
  f1 <- update.formula(f0, . ~ . - (1 | family_id))
  fit1 <- fit_glmer_safe(f1, data, link)
  di1 <- diag_report(fit1)
  if (!isTRUE(di1$is_singular) && isTRUE(di1$converged)) return(fit1)

  #1.15.3 Then drop site (so only participant left)
  f2 <- update.formula(f1, . ~ . - (1 | site_factor))
...
robust_vcov_for <- function(fit, dat) {
  ...
  #1.18.1 For glmer/bglmer, let emmeans use model-based vcov (avoid CR2 handoff errors)
  if (inherits(fit, "merMod")) return(NULL)
  NULL
}

#1.19 Predictions across age WITH CIs (emmeans)
pred_by_age_ci_emm <- function(fit, dat, ages = ages_pred_vec, wave_ref = params$wave_ref) {
  ...
  at_list <- list(
    ...
    sex = levels(dat$sex)[1],
    post_baseline = as.integer(wave_ref != "ses-02A"))
  if (isTRUE(params$include_baseline)) at_list$baseline_status <- 0
  em <- emmeans::emmeans(fit, ~ cluster | age_wave_between, at = at_list)
...
#1.20 Predictions by wave WITH CIs (emmeans)
pred_by_wave_ci_emm <- function(fit, dat) {
  ...
  at_list <- list(
    ...
    sex = levels(dat$sex)[1],
    site_factor = levels(dat$site_factor)[1],
    post_baseline = as.integer(waves != "ses-02A"))
  em <- emmeans::emmeans(fit, ~ wave, at = at_list)
  sm <- as.data.frame(summary(em, infer = TRUE, type = "response"))
  sm %>% dplyr::transmute(wave, pred = prob, lo = asymp.LCL, hi = asymp.UCL)
}

#1.21 Cluster contrasts (OR) via emmeans on link scale
cluster_contrasts_by_age_OR <- function(fit, dat, ages = ages_pred_vec, wave_ref = params$wave_ref) {
  ...
  if (isTRUE(params$include_baseline)) at_list$baseline_status <- 0
  em <- emmeans::emmeans(fit, ~ cluster | age_wave_between, at = at_list)
  ct <- contrast(em, method = "revpairwise")
  sm <- as.data.frame(summary(ct, infer = TRUE, type = "link"))
  lo_nm <- dplyr::case_when("lower.CL" %in% names(sm) ~ "lower.CL",
    "asymp.LCL" %in% names(sm) ~ "asymp.LCL",
    TRUE ~ NA_character_)
  hi_nm <- dplyr::case_when("upper.CL" %in% names(sm) ~ "upper.CL",
    "asymp.UCL" %in% names(sm) ~ "asymp.UCL",
    TRUE ~ NA_character_)
  if (is.na(lo_nm) || is.na(hi_nm)) { sm$CI_lo <- NA_real_; sm$CI_hi <- NA_real_ }
  else { sm$CI_lo <- exp(sm[[lo_nm]]); sm$CI_hi <- exp(sm[[hi_nm]]) }
  sm %>%
    dplyr::mutate(age_years = round(age_wave_between + grand_mean, 2),
      OR = exp(estimate)) %>%
...
#1.23  Weakly-informative normal prior for fixed effects (robust to blme versions)
fixed_effect_prior_for <- function(formula, data, sd = 2.5, common.scale = FALSE) {

  #1.23.1 Design matrix dimension for fixed effects
  X <- model.matrix(lme4::nobars(formula), data)
  k <- ncol(X)

  #1.23.2 Try exported symbol; if not present (older blme), pull from namespace
  normal_fun <- tryCatch(get("normal", getNamespace("blme")), error = function(e) NULL)
  if (is.null(normal_fun)) {
    message("blme::normal() not found; proceeding without a fixed-effect prior.")
    return(NULL)  # bglmer will then behave like glmer
  }
  normal_fun(cov = diag(sd^2, k), common.scale = common.scale)
}
...
for (out in OUTCOMES) {
  ...
  #2.3.5 PRIMARY FITTER: bGLMM to regularize (convergence wasn't possible with previous glmer method)
  if (isTRUE(params$use_bglmer_primary) && requireNamespace("blme", quietly = TRUE)) {
  fam <- if (params$link_primary == "logit") binomial("logit") else binomial("cloglog")
  fx_prior <- fixed_effect_prior_for(form_o, dat_o, sd = 2.5, common.scale = FALSE)
  fit_o <- blme::bglmer(
    form_o, data = dat_o, family = fam,
    fixef.prior = fx_prior,
    control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))
  } else {
    fit_o <- fit_glmer_safe(form_o, dat_o, link = params$link_primary)
  }

  #2.3.6 If singular or shaky convergence, use stepwise RE simplification (drop family, then site)
  fit_o <- simplify_random_effects_me(fit_o, formula(fit_o), dat_o, link = params$link_primary)
  form_o <- formula(fit_o)
  
  #2.3.7 Print final model specification and RE summary
  cat("\n\n### Final model for ", out, "\n")
  print(formula(fit_o))
  print(lme4::VarCorr(fit_o))
  cat("\n\n")
  
  #2.3.8 Age predictions with Wald CIs (fixed-effects delta method)
  preds_primary[[out]] <- pred_by_age_ci_emm(fit_o, dat_o) %>%
    dplyr::mutate(outcome = out, model = params$link_primary, wave_ref = params$wave_ref, .before = 1)

  #2.3.9 Robust cluster OR contrasts by age (true OR on logit scale)
  emm_primary[[out]] <- tryCatch(
    ...
    if (nrow(emm_primary[[out]]) == 0) {
      fe <- tryCatch(lme4::fixef(fit_o), error = function(e) NULL)
      if (!is.null(fe) && "clusterC1" %in% names(fe)) {
        constOR <- exp(unname(fe["clusterC1"]))
        emm_primary[[out]] <- tibble(
          outcome = out, model = params$link_primary,
          age_years = ages_pred_vec,
          contrast = "C1 - C2",
          OR = constOR, CI_lo = NA_real_, CI_hi = NA_real_, p.value = NA_real_)
      }
    }

  #2.3.10 Period (wave) predictions with Wald CIs
  wave_pred <- pred_by_wave_wald(fit_o, dat_o) %>%
    dplyr::mutate(outcome = out, model = params$link_primary, .before = 1)
  if (!exists("preds_wave_all")) preds_wave_all <- list()
  preds_wave_all[[out]] <- wave_pred
  
  #2.3.11 Summaries, predictions, metrics
  wald_primary[[out]] <- safe_or_null(tidy_wald(fit_o) %>% mutate(outcome = out, model = params$link_primary, .before = 1))

  #2.3.12 CR2: cluster by whichever RE(s) we had to drop (family/site). If none dropped, tag as Wald-only.
  cr2_list <- list()
  clusters_needed <- choose_cr2_clusters(fit_o, dat_o)
  if (length(clusters_needed) == 0) {

  #2.3.13.1 No CR2 required; tag a "not_required" row for transparency
  ...
  #2.3.13.2 Store and clean primary fit output
  ...
  #2.3.14 DHARMa diagnostics panels
  ...
  #2.3.15 Sensitivity: complementary log-log
...
#2.3.16 Bind and filter model outputs for tidy downstream use
...
#2.3.17 Save outputs to disk (reproducibility and downstream reporting)
...
plot_pred_age_ci <- function(df, title) {
  ggplot(df, aes(x = age_years, y = pred, color = cluster, fill = cluster)) +
    { if (all(c("lo","hi") %in% names(df))) geom_ribbon(aes(ymin = lo, ymax = hi), alpha = 0.18, colour = NA) else NULL } +
    geom_line(size = 1) +
...
plot_wave_ci <- function(df, title) {
  ggplot(df, aes(x = wave, y = pred)) +
    ...
    { if (all(c("lo","hi") %in% names(df))) geom_errorbar(aes(ymin = lo, ymax = hi), width = 0.15) else NULL } +
    scale_y_continuous(labels = scales::percent_format(accuracy = 0.1), limits = c(0, NA)) +
