...
params:
  ...
  seed: 123
  link_primary: "logit"
  #ages_pred: !r c(12,14,16)    # representative ages (~ the mean) for marginal hazard plots/tables if desired (currently using median ages)
  wave_ref: "ses-04A"
---
...
bd_pp_model <- bd_pp_model %>%
  mutate(
    cluster = forcats::fct_relevel(cluster, cluster_levels),
...
* DTH models the probability of an onset within an interval, conditional on no prior onset. With assessments at distinct waves in ABCD, it is good practice here for us to:
  ...
  * This matches standard practice for event occurrence with panel waves (Singer & Willett; UCLA tutorials)
...
#1.8 Create diagnostics helpers to ensure models meet assumption checks
diag_report <- function(fit) {
  ...
  n_id <- tryCatch(length(lme4::getME(fit, "flist")$participant_id), error = function(e) NA_integer_)
  n_fam <- tryCatch(length(lme4::getME(fit, "flist")$family_id), error = function(e) NA_integer_)
  n_site <- tryCatch(length(lme4::getME(fit, "flist")$site_factor), error = function(e) NA_integer_)
  tibble(is_singular = is_sing, overdispersion_K = overK, n_groups_participant = n_id, n_groups_family = n_fam, n_groups_site = n_site)
}
...
for (out in OUTCOMES) {
  ...
  if (nlevels(dat_o$cluster) < nlevels(bd_pp_model$cluster)) message("Note: some clusters absent in outcome = ", out, "; predictions for absent clusters would be extrapolations.")
  ...
  emm_primary[[out]] <- safe_or_null(cluster_contrasts_by_age(fit_o, dat_o) %>% dplyr::mutate(outcome = out, model = params$link_primary, .before = 1))
  
  #2.4.3 Perform sensitivity analysis of complementary log-log link
...
#3.1 Create a plot helper function
plot_pred <- function(df, title) {
  ggplot(df, aes(x = age_years, y = pred, color = cluster, group = cluster)) +
    geom_line(size = 1.1) +
    geom_point(size = 2) +
    scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
    labs(title = wrap_title(title, 70), x = "Age (years)", y = "Predicted first-onset hazard (per interval)", color = "Cluster") +
...
#5.1 Create a helper function to plot the decile bin calibration for each primary link
...
An optional parsimonious alternative would be to use a single centered age. If we end up needing a simpler reporting model, we could fit:
`event ~ cluster * age_mid_gmc + baseline_status + sex + site_factor + end_wave + (1|id) + (1|family)`; and keep the decomposed model as a sensitivity analysis (this is currently not run here though, to keep runtime minimal)
