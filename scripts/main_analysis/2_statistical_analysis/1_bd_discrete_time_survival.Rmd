...
-   Best practice in longitudinal models is to include both pieces (within & between) to avoid conflating them. This is the within-between (Mundlak/hybrid) decomposition
    ...
    -   We interact cluster x age_mid_between to ask: do cluster differences widen or narrow at older average ages? We retain `age_mid_cwc` as a main effect to absorb within-person aging while wave FEs capture baseline time-since-baseline risk
...
needs_simplify <- function(fit, data) {
  ...
  flag_vif <- !is.na(di$max_VIF) && di$max_VIF > 25
...
cluster_contrasts_by_age <- function(fit, data, ages = ages_pred_vec, wave_ref = params$wave_ref) {
  ...
  ref_list <- list(
    ...
    sex = levels(data$sex)[1],
    site_factor = levels(data$site_factor)[1])
  em <- emmeans::emmeans(fit, ~ cluster | age_mid_between, at = ref_list) # link scale
  ct <- emmeans::contrast(em, method = "revpairwise", adjust = "none")
  sm <- summary(ct, infer = c(TRUE, TRUE))  # link scale, gives estimate & LCL/UCL
  as_tibble(sm) %>%
    mutate(age_years = round(age_mid_between + grand_mean, 2),
      OR = exp(estimate),
      CI_lo = exp(asymp.LCL),
      CI_hi = exp(asymp.UCL)) %>%
...
#1.20 Fixed-wave predictions with 95% CIs via emmeans (response scale)
pred_fixedwave_with_ci <- function(fit, data, ages = ages_pred_vec, wave_ref = params$wave_ref) {
  ...
  at_list <- list(
    ...
    sex = levels(data$sex)[1],
    site_factor = levels(data$site_factor)[1])
  em <- emmeans::emmeans(fit, ~ cluster | age_mid_between, at = at_list, type = "response")
  as_tibble(summary(em)) %>%
    transmute(cluster,
      age_years = round(age_mid_between + grand_mean, 2),
      pred = response, lower = asymp.LCL, upper = asymp.UCL,
      end_wave = wave_ref)
}

#1.21 Wave-aligned predictions (pair each empirical age to its wave) with CIs
pred_aligned_with_ci <- function(fit, data, ages = ages_pred_vec) {
  ...
  for (i in seq_len(k)) {
    at_list <- list(
      ...
      sex = levels(data$sex)[1],
      site_factor = levels(data$site_factor)[1])
    em <- emmeans::emmeans(fit, ~ cluster, at = at_list, type = "response")
    out[[i]] <- as_tibble(summary(em)) %>%
      transmute(cluster,
        end_wave = waves[i],
...
#1.22 Per-interval hazard table by cluster x wave (95% CI via emmeans)
hazard_by_wave_ci <- function(fit, data) {
  at_list <- list(
    ...
    baseline_status = 0,
    end_wave = levels(data$end_wave),
    sex = levels(data$sex)[1], site_factor = levels(data$site_factor)[1]
  )
  em <- emmeans::emmeans(fit, ~ cluster | end_wave, at = at_list, type = "response")
  as_tibble(summary(em)) %>%
    transmute(end_wave, cluster, hazard = response, lo = asymp.LCL, hi = asymp.UCL)
}

#1.23 Cumulative risk by wave with bootstrap CIs (parametric via bootMer)
cumrisk_by_wave_ci <- function(fit, data, B = 300) {
  ...
  nd <- expand.grid(
    ...
    site_factor = levels(data$site_factor)[1]) %>% 
    arrange(cluster, end_wave)

  #1.23.2 Point predictions
  haz_hat <- as.numeric(predict(fit, newdata = nd, type = "response", re.form = NA))

  #1.23.3 Cumrisk from hazards helper
  to_cum <- function(h) {
    df <- nd
    df$haz <- h
    df %>%
      group_by(cluster) %>%
      mutate(surv = cumprod(1 - haz), cumrisk = 1 - surv) %>%
      ungroup() %>% select(end_wave, cluster, cumrisk)
  }

  #1.23.3 Bootstrap draws
  if (inherits(try(lme4::bootMer, silent = TRUE), "function")) {
    sim <- lme4::bootMer(
      fit,
    ...
    #1.23.4 Build CIs at each row of nd
    cis <- lapply(seq_len(nrow(nd)), function(j) {
      qs <- quantile(sims[[j]], probs = c(0.025, 0.975), na.rm = TRUE)
      tibble(row = j, lohaz = qs[[1]], hihaz = qs[[2]])
    }) %>% bind_rows()
    nd2 <- nd %>% mutate(row = row_number(), haz = haz_hat) %>% left_join(cis, by = "row")
    
    #1.23.5 compute cumrisk for each bootstrap by path (approx by plug-in at each wave) using simple approximation: transform lo/hi hazards to lo/hi cumrisk via same mapping
    lo <- to_cum(nd2$lohaz) %>% rename(cumrisk_lo = cumrisk)
    hi <- to_cum(nd2$hihaz) %>% rename(cumrisk_hi = cumrisk)
    pt <- to_cum(nd2$haz) %>% rename(cumrisk = cumrisk)
    pt %>% left_join(lo, by = c("end_wave","cluster")) %>% left_join(hi, by = c("end_wave","cluster"))
  } else {
    warning("bootMer not available; returning point estimates only.")
    to_cum(haz_hat) %>% mutate(cumrisk_lo = NA_real_, cumrisk_hi = NA_real_)
  }
}
...
#2.3 Establish and implement the base formula
form_full <- build_formula(link = params$link_primary)

#2.3.1 Create a helper to drop cluster:age_mid_between only (keep main effects)
...
for (out in OUTCOMES) {
  ...
  #2.4.1 Start with full interaction
  form_o <- form_full
  ...
  # 2.4.1.1 If BD-II shows big collinearity, drop the interaction
  ...
  # 2.4.1.2 If BD-NOS or Any BSD still singular after simplification, force participant-only RE
  ...
  #2.4.2 If diagnostics demand, simplify once (drop cluster:age_mid_between)
  if (needs_simplify(fit_o, dat_o)) {
    ...
    # Keep the simplified fit only if it improves diagnostics (not singular + lower VIF or more events per param)
    di0 <- diag_report(fit_o); di1 <- diag_report(fit_o_s)
    keep_simple <- isTRUE(di1$is_singular) < isTRUE(di0$is_singular) ||
      (!is.na(di1$max_VIF) && !is.na(di0$max_VIF) && di1$max_VIF < di0$max_VIF)
    if (keep_simple) {
      fit_o <- fit_o_s
      form_o <- form_o_s
  ...
  cum_tbl <- safe_or_null(cumrisk_by_wave_ci(fit_o, dat_o, B = 300))
  if (!is.null(cum_tbl)) {
  ...
  #2.4.3.6 Emmeans cluster contrasts only when identifiable (non-singular and ≥2 clusters)
  ok_emm <- isTRUE(tryCatch(!lme4::isSingular(fit_o, tol = 1e-4), error = function(e) FALSE)) && nlevels(dat_o$cluster) >= 2
...
## Sensitivity (rare outcomes): Firth fixed-effects; glmmTMB + OLRE if needed
sens_dir <- file.path(OUT_DIR, "sensitivity"); dir.create(sens_dir, showWarnings = FALSE)
SENS <- c("bipolar_I","bipolar_II")

for (out in SENS) {
  dat_o <- bd_pp_model %>% dplyr::filter(outcome == out) %>% droplevels()

  # -- Firth penalized logit (fixed-effects only), if logistf available
  if (requireNamespace("logistf", quietly = TRUE)) {
    f_form <- event ~ cluster + age_mid_between + age_mid_cwc + sex + end_wave
    fit_firth <- try(logistf::logistf(f_form, data = dat_o), silent = TRUE)
    if (!inherits(fit_firth, "try-error")) {
      broom::tidy(fit_firth, conf.int = TRUE, exponentiate = TRUE) %>%
        readr::write_csv(file.path(sens_dir, paste0("firth_", out, ".csv")))
    }
  }

  # -- glmmTMB with observation-level RE if DHARMa dispersion flagged
  # create obs_id
  dat_o$obs_id <- seq_len(nrow(dat_o))
  try_disp <- dharma_tests_all %>% dplyr::filter(outcome == out) %>% dplyr::pull(p_dispersion)
  if (requireNamespace("glmmTMB", quietly = TRUE) && length(try_disp) && !is.na(try_disp) && try_disp < 0.05) {
    tmb_form <- event ~ cluster * age_mid_between + age_mid_cwc + sex + end_wave + (1|participant_id) + (1|obs_id)
    fit_tmb <- try(glmmTMB::glmmTMB(tmb_form, data = dat_o, family = binomial(link = "logit")), silent = TRUE)
    if (!inherits(fit_tmb, "try-error")) {
      broom.mixed::tidy(fit_tmb, effects = "fixed", conf.int = TRUE, conf.method = "Wald") %>%
        dplyr::mutate(OR = exp(estimate), CI_lo = exp(conf.low), CI_hi = exp(conf.high)) %>%
        readr::write_csv(file.path(sens_dir, paste0("glmmTMB_olre_", out, ".csv")))
    }
  }
}
...
#2.5.2 Save additional outputs
cal_summ_all <- dplyr::bind_rows(purrr::compact(cal_summaries))
haz_tables_all <- dplyr::bind_rows(purrr::compact(hazard_tables))
cumrisk_tables_all <- dplyr::bind_rows(purrr::compact(cumrisk_tables))
...
readr::write_csv(pred_all_aligned, file.path(OUT_DIR, "bd_dth_preds_by_age_aligned.csv"))
readr::write_csv(pred_all_fixedwave, file.path(OUT_DIR, "bd_dth_preds_by_age_fixed_wave.csv"))
...
plot_pred_fixed <- function(df, title) {
  ggplot(df, aes(x = age_years, y = pred, color = cluster, group = cluster)) +
    {if (all(c("lower","upper") %in% names(df)))
      geom_ribbon(aes(ymin = lower, ymax = upper, fill = cluster), alpha = 0.15, color = NA)} +
    geom_line(size = 1.0) + geom_point(size = 1.8) +
    scale_y_continuous(labels = scales::percent_format(accuracy = 0.01)) +
    labs(title = wrap_title(title, 70),
         x = "Age (years)", y = "Predicted first-onset hazard (per interval)", color = "Cluster", fill = "Cluster") +
    theme_minimal(base_size = 9) +
    theme(legend.position = "bottom", legend.box = "horizontal", plot.title = element_text(lineheight = 0.95)) +
    guides(color = guide_legend(nrow = 1), fill = guide_legend(nrow = 1))
}

#3.1.2 Wave - age aligned predictions
plot_pred_aligned <- function(df, title) {
  ggplot(df, aes(x = end_wave, y = pred, color = cluster, group = cluster)) +
    ...
    scale_y_continuous(labels = scales::percent_format(accuracy = 0.01)) +
    labs(title = wrap_title(title, 70),
         x = "Wave (interval end)", y = "Predicted first-onset hazard (per interval)", color = "Cluster", fill = "Cluster") +
    theme_minimal(base_size = 9) +
    theme(legend.position = "bottom", legend.box = "horizontal", plot.title = element_text(lineheight = 0.95)) +
    guides(color = guide_legend(nrow = 1), fill = guide_legend(nrow = 1))
}
