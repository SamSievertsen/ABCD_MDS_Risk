...
tidy_cr2 <- function(fit, cluster_var = NULL) {
  ...
  if (inherits(ct, "try-error")) {
    out <- broom.mixed::tidy(fit, effects = "fixed", conf.int = TRUE, conf.method = "Wald") %>%
      dplyr::mutate(term = as.character(term), OR = exp(estimate),
  ...
  ctt %>%
    dplyr::mutate(term = as.character(term),
      OR = exp(beta), CI_lo = exp(CI_L), CI_hi = exp(CI_U)) %>%
    dplyr::select(term, estimate = beta, SE = SE, df, OR, CI_lo, CI_hi, p.value = p_any)
}
...
cluster_contrasts_by_age <- function(fit, data, ages = ages_pred_vec, wave_ref = params$wave_ref) {
  ...
  if (!inherits(mf_names, "try-error")) {
    if ("age_mid_cwc_orth" %in% mf_names) ref_list$age_mid_cwc <- NULL
    if (!"age_mid_cwc_orth" %in% mf_names) ref_list$age_mid_cwc_orth <- NULL
...
#1.9.2 Wave-specific cluster contrasts (OR) at wave-median ages, robust to wave_num/age_mid_cwc_orth
cluster_contrasts_by_wave_aligned <- function(fit, data) {
  ...
  for (i in seq_along(waves)) {
    ...
    if (!inherits(mf_names, "try-error")) {
      ...
      if ("age_mid_cwc_orth" %in% mf_names) at$age_mid_cwc <- NULL else at$age_mid_cwc_orth <- NULL
...
hazard_by_wave_ci <- function(fit, data) {
  at_list <- list(
    cluster = levels(data$cluster),
    age_mid_between = 0, age_mid_cwc = 0, age_mid_cwc_orth = 0,
    baseline_status = 0,
    end_wave = levels(data$end_wave),
...
#1.32 Compare nested models for an interaction (with bootstrap + CV)
#compare_models <- function(form_small, form_big, dat, link = params$link_primary, B = 500, kfold = 5) {
  #fit_s <- fit_glmer_safe(form_small, dat, link = link)
  #fit_b <- fit_glmer_safe(form_big,   dat, link = link)

  #1.32.1 Likelihood: AIC/BIC (negative favors BIG)
  #dAIC <- AIC(fit_b) - AIC(fit_s)
  #dBIC <- BIC(fit_b) - BIC(fit_s)

  #1.32.2 Bootstrap LRT under SMALL
  #set.seed(params$seed)
  #D_obs <- as.numeric(-2*(logLik(fit_s) - logLik(fit_b)))
  #D_sim <- replicate(B, {
    #ysim <- unlist(simulate(fit_s, nsim = 1))
    #ds <- dat; ds$event <- ysim
    #f1 <- safe_or_null(fit_glmer_safe(form_small, ds, link = link))
    #f2 <- safe_or_null(fit_glmer_safe(form_big, ds, link = link))
    #if (is.null(f1) || is.null(f2)) return(NA_real_)
    #as.numeric(-2*(logLik(f1) - logLik(f2)))
  #})
  #p_boot <- mean(D_sim[is.finite(D_sim)] >= D_obs)

  #1.32.3 Participant-level K-fold CV (log loss, Brier, per-wave×cluster RMSE)
  #ids <- unique(dat$participant_id)
  #set.seed(params$seed)
  #folds <- split(sample(ids), cut(seq_along(ids), kfold, labels = FALSE))
  #eval_cv <- function(form) {
    #LL <- BR <- RM <- numeric()
    #for (id_set in folds) {
      #tr <- dat[!(dat$participant_id %in% id_set), ]
      #te <- dat[ (dat$participant_id %in% id_set), ]
      #fm <- safe_or_null(fit_glmer_safe(form, tr, link = link)); if (is.null(fm)) next
      #pr <- as.numeric(predict(fm, newdata = te, type = "response", re.form = NA))
      #LL <- c(LL, log_loss(te$event, pr))
      #BR <- c(BR, brier_score(te$event, pr))
      #emp <- te %>% dplyr::group_by(end_wave, cluster) %>%
             #dplyr::summarise(emp = mean(event==1L), .groups="drop")
      #pd <- te %>% dplyr::mutate(pred = pr) %>%
             #dplyr::group_by(end_wave, cluster) %>%
             #dplyr::summarise(pred = mean(pred), .groups="drop")
      #j <- dplyr::left_join(emp, pd, by = c("end_wave","cluster"))
      #RM <- c(RM, sqrt(mean((j$emp - j$pred)^2, na.rm = TRUE)))
    #}
    #list(logloss = mean(LL), brier = mean(BR), rmse_wave = mean(RM))
  #}
  #cv_s <- eval_cv(form_small); cv_b <- eval_cv(form_big)
  #list(
    #dAIC = dAIC, dBIC = dBIC, D_obs = D_obs, p_boot = p_boot,
    #cv_small = cv_s, cv_big = cv_b,
    #diag_small = diag_report(fit_s), diag_big = diag_report(fit_b))
#}
...
for (out in OUTCOMES) {
  ...
  if (identical(out, "bipolar_II")) {
    message("BD-II: applying staged stabilization (participant-only RE; orthogonalize age within wave; fallbacks).")
  
    #2.4.0.1 Empirical wave index (0,1,2) in case we need a 1-df baseline trend
    dat_o <- dat_o %>% mutate(wave_num = as.integer(end_wave) - 1L)
  
    #2.4.0.2 Orthogonalize within-person age to wave FEs using the actual wave structure (this is equivalent to within-wave centering relative to wave means)
    dat_o <- dat_o %>%
      mutate(age_mid_cwc_orth = resid_on_factor(age_mid_cwc, end_wave))
    ...
    f_ORTH <- event ~ cluster + age_mid_between + age_mid_cwc_orth  + sex + end_wave + (1|participant_id)
    ...
    try_one <- function(form, dat = dat_o) {
      ...
      list(fit = fit,
        ok = ok_convergence(fit) && (max_adj_vif(fit) <= VIF_THRESH),
        adj_vif = max_adj_vif(fit))
    }
    cand_order <- list(
      RE1 = list(form = f_RE1, dat = dat_o, note = "Participant-only RE"),
      ORTH = list(form = f_ORTH, dat = dat_o, note = "age_mid_cwc orthogonalized to end_wave"),
    ...
    chosen <- NULL; tried <- list()
    for (nm in names(cand_order)) {
      res <- try_one(cand_order[[nm]]$form, cand_order[[nm]]$dat)
      if (is.null(res)) next
      tried[[nm]] <- res$adj_vif
      message("BD-II candidate ", nm, " adj.VIF=", sprintf("%.2f", res$adj_vif), " | converged=", ok_convergence(res$fit), " (", cand_order[[nm]]$note, ")")
      if (isTRUE(res$ok)) { chosen <- res$fit; break }
  }
  
    #2.4.0.5 If none met both criteria, try weakly-informative priors on the most principled spec (ORTH)
    if (is.null(chosen)) {
      if (requireNamespace("blme", quietly = TRUE)) {
        message("BD-II: trying bglmer with weak priors on ORTH spec.")
        chosen <- safe_or_null(
          blme::bglmer(f_ORTH, data = dat_o, family = binomial(),
            fixef.prior = blme::normal(scale = 2.5),
            control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))))
        if (!is.null(chosen)) {
          message("BD-II bglmer adj.VIF=", sprintf("%.2f", max_adj_vif(chosen)), " | converged=", ok_convergence(chosen))
        }
      } else {
