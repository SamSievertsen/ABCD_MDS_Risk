...
simplify_random_effects <- function(fit, form, data, link) {
  ...
  #1.13.2.1 Try dropping (1|site_factor) then (1|family_id); never drop (1|participant_id)
  candidates <- c("site_factor", "family_id")
  best_fit  <- fit
  best_form <- form
...
for (out in OUTCOMES) {
  ...
  preds_primary_fixedwave[[out]] <- safe_or_null(
    pred_grid_by_age(fit_o, dat_o, ages = ages_pred_vec, wave_ref = params$wave_ref, align_waves = FALSE) %>%
  ...
  # emmeans cluster contrasts only when identifiable (non-singular and ≥2 clusters)
...
- **Cumulative first-onset risk** (third panel): Converts per-interval hazards into the probability of having had a first onset by each wave
  ...
  - This is the “when do onsets happen” view most would expect - rising curves that separate by cluster indicate earlier
  and/or more frequent onsets in the higher-risk cluster
...
plot_pred_fixed <- function(df, title) {
  ggplot(df, aes(x = age_years, y = pred, color = cluster, group = cluster)) +
    ...
    labs(title = wrap_title(title, 70),
         x = "Age (years)", y = "Predicted first-onset hazard (per interval)", color = "Cluster") +
...
plot_pred_aligned <- function(df, title) {
  ggplot(df, aes(x = end_wave, y = pred, color = cluster, group = cluster)) +
    ...
    scale_y_continuous(labels = scales::percent_format(accuracy = 0.01)) +
    labs(title = wrap_title(title, 70),
         x = "Wave (interval end)", y = "Predicted first-onset hazard (per interval)", color = "Cluster") +
    theme_minimal(base_size = 9) +
    theme(legend.position = "bottom", legend.box = "horizontal", plot.title = element_text(lineheight = 0.95)) +
    guides(color = guide_legend(nrow = 1))
}
