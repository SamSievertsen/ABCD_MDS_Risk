...
-   Fixed effects (core):
    ...
    -   `cluster * age_mid_between + age_mid_cwc + baseline_status + sex + site_factor + wave FEs`
    -   `baseline_status` excludes at-risk time for baseline-positive cases and adjusts residual differences at first modeled wave
...
#2.3.1 Optional flags to simplify sparse outcomes
drop_interaction_outcomes <- c("bipolar_I","bipolar_II")

#2.3.2 Helper to drop cluster:age_mid_between only (keep main effects)
strip_interaction <- function(f) {
  update.formula(f, . ~ . - cluster:age_mid_between)
}

#2.4 Loop formula per outcome
for (out in OUTCOMES) {
  
  #2.4.1 Begin the primary fit and levels checks
  ...
  #2.4.1.1 Optional simplification for sparse outcomes
  form_o <- form
  if (out %in% drop_interaction_outcomes) {
    message("Simplifying fixed effects (drop cluster:age_mid_between) for sparse outcome = ", out, ".")
    form_o <- strip_interaction(form)
  }
  
  #2.4.2 Establish primary fit (use simplified form_o when flagged)
  ...
  #2.4.3 Summaries and predictions (wave-aligned ages)
  ...
  preds_primary[[out]] <- safe_or_null(
    pred_grid_by_age(fit_o, dat_o, ages = ages_pred_vec, align_waves = TRUE) %>%
  ...
  #2.4.4 Export diagnostics (check_model, DHARMa)
  ...
  #2.4.5 emmeans contrasts only when identifiable
  ...
  #2.4.6 Sensitivity: complementary log-log with offset(log_dt) (use same fixed-effect spec as primary via form_o)
...
## 3. Marginal hazard by cluster across age (ages paired to waves; within-age = 0) ##

#3.1.1 Plot helper: clearer labels, wrapped title, compact legend, finer y-axis ticks
...
#3.1.2 Execute one plot panel per outcome (primary link)
...
#3.1.3 Facet wrap the plots
wrap_plots(plots, ncol = 2)

#3.1.4 Quick QC: overlay empirical per-wave hazards (points) on model lines for any_bsd
empirical_haz <- function(df) {
  df %>%
    group_by(outcome, end_wave, cluster) %>%
    summarise(emp = mean(event == 1L), .groups = "drop")
}
qc_plots <- lapply(OUTCOMES, function(out) {
  dfm <- pred_all %>% filter(outcome == out, model == params$link_primary)
...
#3.2 Generate cumulative incidence by wave (reference profile)
#3.2.1 Create a helper function to generate the plots
...
#3.2.2 Print the cumulative incidence by wave plots
...
plot_cal <- function(df, title) {
  ...
  ggplot(df, aes(x = pred_mean, y = obs_mean)) +
    ...
    scale_x_continuous(limits = c(0, 1), labels = scales::percent_format(accuracy = 0.01)) +
    scale_y_continuous(limits = c(0, 1), labels = scales::percent_format(accuracy = 0.01)) +
    coord_equal() +
    labs(title = wrap_title(title, 70),
